...
def decide_gateway_owner() -> dict[str, Any]:
    """Decide which gateway should be treated as active right now.

    Prefer the explicit runtime owner when available, then fall back to live
    probes so health/fallback flows can avoid restarting the wrong side.
    """
    owner = build_runtime_owner()
    windows_process = detect_windows_openclaw_process()
    wsl_gateway_ok = _gateway_ok()
    runtime_state = str(owner.get("runtime_state") or "")
    wsl_live = runtime_state == STATE_WSL_LIVE
    rollback_windows = runtime_state == STATE_ROLLBACK_WINDOWS

    if wsl_live and wsl_gateway_ok:
        active = "wsl"
    elif rollback_windows and bool(windows_process.get("running")):
        active = "native"
    elif wsl_gateway_ok and not bool(windows_process.get("running")):
        active = "wsl"
    elif bool(windows_process.get("running")) and not wsl_gateway_ok:
        active = "native"
    elif wsl_gateway_ok and bool(windows_process.get("running")):
        active = "conflict"
    else:
        active = "unknown"
    ...
    return {
        ...
        "wsl_gateway_ok": wsl_gateway_ok,
        "windows_openclaw_running": bool(windows_process.get("running")),
...
def detect_windows_openclaw_process() -> dict[str, Any]:
    ...
    try:
        ...
        return {"ok": False, "running": False, "reason": "timeout", "processes": []}
