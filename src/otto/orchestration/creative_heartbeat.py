...
def write_heartbeat_manifest() -> dict[str, Any]:
    manifest = {
        ...
        "tools": [
            ...
            {"name": "otto.heartbeat_readiness", "command": "python3 -m otto.cli heartbeat-readiness --strict", "risk": "read_only"},
        ],
    }
    return ensure_json(heartbeat_manifest_path(), manifest)


def run_creative_heartbeat(*, dry_run: bool = True, explain: bool = False) -> dict[str, Any]:
    ...
    autonomous = run_autonomous_heartbeat(dry_run=dry_run)
    result = {
        "ok": True,
    ...
    if explain:
        result["explain"] = {
            "review_gated": True,
            "candidate_only": True,
            "why_no_autopublish": "safety.auto_publish=false",
            "why_no_raw_qmd": "safety.auto_qmd_index_raw=false",
            "why_no_unreviewed_vault_write": "safety.auto_vault_write_unreviewed=false",
            "heartbeat_jobs": policy["cadence"],
            "expected_outputs": [
                "state/creative/songforge/autonomous_song_candidates.jsonl",
                "state/research/autonomous_paper_candidates.jsonl",
