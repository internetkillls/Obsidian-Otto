...
def build_runtime_smoke(*, gateway_port: int = 18790, write: bool = True, strict: bool = False) -> dict[str, Any]:
    ...
    soul_health = build_soul_health()
    heartbeat_readiness = build_heartbeat_readiness(strict=strict, write=write, run_dry_runs=False)
    ...
    if runtime_state in {STATE_WSL_SHADOW_MEMORY, STATE_WSL_SHADOW_GATEWAY}:
        ...
        gates["soul_otto_realm_exists_live"] = bool((soul_health.get("checks") or {}).get("otto_realm_exists"))
        gates["soul_profile_snapshot_exists_live"] = bool((soul_health.get("checks") or {}).get("profile_snapshot_exists"))
        gates["soul_heartbeats_dir_exists_live"] = bool((soul_health.get("checks") or {}).get("heartbeats_dir_exists"))
        gates["qmd_manifest_has_soul_sources_live"] = bool((soul_health.get("checks") or {}).get("qmd_manifest_includes_soul_sources"))
    ...
    if strict:
        gates["soul_manifest_exists"] = bool((soul_health.get("checks") or {}).get("soul_manifest_exists"))
        gates["soul_repo_root_wsl_exists"] = bool((soul_health.get("checks") or {}).get("repo_root_wsl_exists"))
        gates["soul_vault_root_wsl_exists"] = bool((soul_health.get("checks") or {}).get("vault_root_wsl_exists"))
        gates["soul_otto_realm_exists"] = bool((soul_health.get("checks") or {}).get("otto_realm_exists"))
        gates["soul_profile_snapshot_exists"] = bool((soul_health.get("checks") or {}).get("profile_snapshot_exists"))
        gates["soul_heartbeats_dir_exists"] = bool((soul_health.get("checks") or {}).get("heartbeats_dir_exists"))
        gates["soul_brain_dir_exists"] = bool((soul_health.get("checks") or {}).get("brain_dir_exists"))
        gates["qmd_manifest_includes_soul_sources"] = bool((soul_health.get("checks") or {}).get("qmd_manifest_includes_soul_sources"))
        gates["autonomous_generation_policy"] = bool(autonomy_policy.get("ok"))
        context_pack = build_openclaw_context_pack(task="runtime-smoke-strict")
        gates["context_pack_has_soul_section"] = bool((context_pack or {}).get("soul"))
        gates["heartbeat_readiness_strict"] = bool(heartbeat_readiness.get("ok"))
        if bool((soul_health.get("checks") or {}).get("legacy_wrong_root_exists")) and bool((soul_health.get("checks") or {}).get("otto_realm_exists")):
            warnings.append("legacy_wrong_root_present_but_canonical_healthy")
    ...
    result = {
        ...
        "soul_identity": {
            "soul_manifest_exists": "green" if (soul_health.get("checks") or {}).get("soul_manifest_exists") else "red",
            "repo_root_wsl_exists": "green" if (soul_health.get("checks") or {}).get("repo_root_wsl_exists") else "red",
            "vault_root_wsl_exists": "green" if (soul_health.get("checks") or {}).get("vault_root_wsl_exists") else "red",
            "otto_realm_exists": "green" if (soul_health.get("checks") or {}).get("otto_realm_exists") else "red",
            "profile_snapshot_exists": "green" if (soul_health.get("checks") or {}).get("profile_snapshot_exists") else "red",
            "heartbeats_dir_exists": "green" if (soul_health.get("checks") or {}).get("heartbeats_dir_exists") else "red",
            "brain_dir_exists": "green" if (soul_health.get("checks") or {}).get("brain_dir_exists") else "red",
            "qmd_manifest_includes_soul_sources": "green" if (soul_health.get("checks") or {}).get("qmd_manifest_includes_soul_sources") else "red",
            "qmd_search_finds_profile_or_heartbeat": "green" if (soul_health.get("checks") or {}).get("qmd_search_finds_profile_or_heartbeat") else "red",
            "context_pack_has_soul_section": "green"
            if bool((read_json(runtime_smoke_path().parents[1] / "openclaw" / "context_pack_v1.json", default={}) or {}).get("soul"))
            else "red",
        },
        "heartbeat_readiness": {
            "state": heartbeat_readiness.get("state"),
            "strict_ready": bool(heartbeat_readiness.get("ok")),
            "discovery": "green" if all((heartbeat_readiness.get("discovery") or {}).values()) else "red",
            "dry_run_contract": "green" if all((heartbeat_readiness.get("dry_run_checks") or {}).values()) else "red",
            "bridge": "green" if all((heartbeat_readiness.get("bridge_checks") or {}).values()) else "red",
            "soul_agent_skill": "green" if all((heartbeat_readiness.get("soul_agent_skill_checks") or {}).values()) else "red",
            "cron_plan": "green" if all((heartbeat_readiness.get("cron_checks") or {}).values()) else "red",
            "safety": "green" if all((heartbeat_readiness.get("safety_checks") or {}).values()) else "red",
        },
        "creative_forge": {
