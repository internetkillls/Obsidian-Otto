...
from .orchestration.runtime_owner import STATE_WSL_LIVE, build_runtime_owner
...
def fallback_to_native() -> dict[str, Any]:
    ...
    result = {
        "ok": bool(native_restart.get("ok") or native_probe.get("ok")),
        "state": "NATIVE_GATEWAY_ACTIVE" if (native_restart.get("ok") or native_probe.get("ok")) else "NATIVE_GATEWAY_UNHEALTHY",
        "created_at": now_iso(),
        ...
        "native_probe": {"ok": native_probe.get("ok"), "reason": native_probe.get("reason"), "port": native_probe.get("port")},
