...
from .adapters.qmd.manifest import DEFAULT_QMD_COMMAND, build_qmd_manifest, qmd_manifest_health
...
def _path_join_posix(base: str, rel: str) -> str:
    return f"{base.rstrip('/')}/{rel.lstrip('/')}"


def _expand_qmd_manifest_source_for_openclaw(source: dict[str, Any]) -> list[dict[str, str]]:
    source_id = str(source.get("id") or source.get("name") or "source")
    base = str(source.get("path_wsl") or source.get("path") or "").rstrip("/")
    if not base:
        return []
    include_globs = [str(item).strip("/") for item in (source.get("include_globs") or []) if str(item).strip("/")]
    if not include_globs:
        return [{"name": source_id, "path": base, "pattern": str(source.get("pattern") or "**/*.md")}]

    expanded: list[dict[str, str]] = []
    for index, include_glob in enumerate(include_globs, start=1):
        parts = include_glob.split("/")
        if len(parts) > 1 and "*" not in parts[0]:
            path = _path_join_posix(base, parts[0])
            pattern = "/".join(parts[1:]) or "**/*"
        else:
            path = base
...
def _qmd_manifest_paths_for_openclaw() -> list[dict[str, str]]:
    ...
    for source in manifest.get("sources", []):
        if not isinstance(source, dict):
            continue
        for item in _expand_qmd_manifest_source_for_openclaw(source):
...
def _with_otto_contract(config: dict[str, Any]) -> dict[str, Any]:
    ...
    heartbeat["every"] = "2m"
    qmd_paths = _qmd_manifest_paths_for_openclaw()
    if qmd_paths:
        ...
        manifest = build_qmd_manifest()
        if sys.platform != "win32":
            qmd["command"] = str(manifest.get("qmd_command") or DEFAULT_QMD_COMMAND)
        else:
            qmd.setdefault("command", str((config.get("memory") or {}).get("qmd", {}).get("command") or DEFAULT_QMD_COMMAND))
        qmd["paths"] = qmd_paths
    return merged
...
def _count_qmd_source_matches(source_path: Path, pattern: str, *, limit: int = 10000) -> int:
    if not source_path.exists():
        return 0
    if source_path.is_file():
        return 1
    normalized_pattern = (pattern or "**/*.md").replace("\\", "/")
    count = 0
    for candidate in source_path.glob(normalized_pattern):
...
def build_qmd_index_health(live_config_path: Path | None = None) -> dict[str, Any]:
    ...
    for src in _extract_qmd_sources(chosen_config):
        ...
        pattern = str(src.get("pattern") or "**/*.md")
        src_path = _qmd_source_host_path(raw_path)
        exists = src_path.exists()
        md_count = _count_qmd_source_matches(src_path, pattern) if exists else 0
    ...
    soul_groups = (registry_manifest or {}).get("soul_control_plane", {})
    if isinstance(soul_groups, bool):
        control_plane_present = soul_groups
        soul_identity_present = soul_groups
    elif isinstance(soul_groups, dict):
        control_plane_entry = soul_groups.get("control_plane_identity")
        soul_identity_entry = soul_groups.get("soul_identity")
        if isinstance(control_plane_entry, dict):
            control_plane_present = bool(control_plane_entry.get("present"))
        else:
            control_plane_present = bool(control_plane_entry)
        if isinstance(soul_identity_entry, dict):
            soul_identity_present = bool(soul_identity_entry.get("present"))
        else:
            soul_identity_present = bool(soul_identity_entry)
    else:
        control_plane_present = False
        soul_identity_present = False
    return {
        "ok": all_ok,
        ...
        "manifest": registry_manifest,
        "soul_control_plane_status": (registry_manifest or {}).get("soul_control_plane", {}),
        "soul_source_groups": {
            "control_plane_identity_present": control_plane_present,
            "soul_identity_present": soul_identity_present,
            "status": "green" if (control_plane_present and soul_identity_present) else "red",
        },
        "qmd_manifest_has_control_plane_identity": control_plane_present,
        "qmd_manifest_has_soul_identity": soul_identity_present,
...
def run_qmd_index_refresh(timeout_seconds: int = 60) -> dict[str, Any]:
    ...
    if cooldown_remaining > 0 and not health.get("ok") and str(refresh_state.get("failure_kind") or "") in {"command-failed", "timeout"}:
