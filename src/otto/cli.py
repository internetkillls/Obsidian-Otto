...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_norm = sub.add_parser("normalize-event", help="Normalize one raw datum into a SilverEvent")
    p_norm.add_argument("--source", required=True)
    p_norm.add_argument("--text", default=None)
    p_norm.add_argument("--kind", default="idea_fragment")
    p_norm.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=True)
    p_fv = sub.add_parser("feature-vectorize", help="Create one Otto feature vector from a SilverEvent")
    p_fv.add_argument("--event-id", required=True)
    p_fv.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=True)
    p_s2c = sub.add_parser("silver-to-candidate", help="Promote one SilverEvent into a CandidateInsight")
    p_s2c.add_argument("--event-id", required=True)
    p_s2c.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=True)
    p_enrich = sub.add_parser("enrich-candidate", help="Enrich one CandidateInsight")
    p_enrich.add_argument("--candidate-id", required=True)
    p_enrich.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=True)
    p_gold_ready = sub.add_parser("gold-readiness", help="Score one enriched candidate for Gold")
    p_gold_ready.add_argument("--candidate-id", required=True)
    p_gold_ready.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=False)
    p_compile_gold = sub.add_parser("compile-gold", help="Compile one reviewed candidate into Gold")
    p_compile_gold.add_argument("--candidate-id", required=True)
    p_compile_gold.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=True)
    sub.add_parser("gold-audit", help="Audit Gold, candidate, and QMD boundary invariants")
    p_rehab_audit = sub.add_parser("gold-rehab-audit", help="Audit the Silver corpus for rehabilitation readiness")
    p_rehab_audit.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=True)
    p_rehab_audit.add_argument("--limit", type=int, default=None)
    p_rehab_score = sub.add_parser("gold-rehab-score", help="Score rehabilitation readiness")
    p_rehab_score.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=True)
    p_rehab_score.add_argument("--limit", type=int, default=None)
    p_rehab_frontmatter = sub.add_parser("gold-rehab-plan-frontmatter", help="Plan frontmatter-safe rehab patches")
    p_rehab_frontmatter.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=True)
    p_rehab_frontmatter.add_argument("--limit", type=int, default=None)
    p_rehab_append = sub.add_parser("gold-rehab-plan-append", help="Plan semantic auto-append enrichment")
    p_rehab_append.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=True)
    p_rehab_append.add_argument("--limit", type=int, default=None)
    p_rehab_apply = sub.add_parser("gold-rehab-apply-safe", help="Apply safe marked rehab enrichment")
    p_rehab_apply.add_argument("--risk-max", default="R1_LOW_RISK_SEMANTIC")
    p_rehab_apply.add_argument("--batch-size", type=int, default=10)
    sub.add_parser("gold-rehab-review-needed", help="Show rehab items that require review")
    sub.add_parser("gold-rehab-discuss-later", help="Show rehab items deferred for discussion")
    p_rehab_rollback = sub.add_parser("gold-rehab-rollback", help="Preview rollback for one rehab patch")
    p_rehab_rollback.add_argument("--patch-id", required=True)
    p_rehab_rollback.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=True)
    sub.add_parser("gold-rehab-status", help="Summarize rehab corridor state")
    sub.add_parser("training-candidates", help="Build training candidates from Gold")
    sub.add_parser("training-export-manifest", help="Write the training dataset manifest")
    ...
    sub.add_parser("soul-manifest", help="Generate the soul identity manifest")
    sub.add_parser("soul-health", help="Check soul identity and heartbeat path health")
    p_soul_rehydrate = sub.add_parser("soul-rehydrate", help="Rehydrate soul identity state")
    p_soul_rehydrate.add_argument("--dry-run", action="store_true")
    p_soul_rehydrate.add_argument("--write", action="store_true")
    sub.add_parser("heartbeat-root-audit", help="Audit root control docs and canonical soul paths")
    sub.add_parser("qmd-soul-audit", help="Audit QMD soul/control-plane retrievability")
    p_hb_ready = sub.add_parser("heartbeat-readiness", help="Run proactive heartbeat readiness checks")
    p_hb_ready.add_argument("--strict", action="store_true")
    sub.add_parser("heartbeat-dry-run-all", help="Run all heartbeat candidate dry-runs")
    p_hb_router = sub.add_parser("heartbeat-router-test", help="Route one Telegram/OpenClaw message into heartbeat commands")
    p_hb_router.add_argument("--message", required=True)
    sub.add_parser("next-due-jobs", help="Show scheduled heartbeat jobs and which are due now")
    sub.add_parser("cron-plan", help="Write planned heartbeat jobs")
    p_cron_render = sub.add_parser("cron-render", help="Render cron plan file in disabled mode")
    p_cron_render.add_argument("--disabled", action="store_true")
    sub.add_parser("cron-verify", help="Verify cron plan and safety policy gates")
    ...
    p_feedback_ingest = sub.add_parser("feedback-ingest", help="Ingest private feedback into creative priors")
    p_feedback_ingest.add_argument("--song-id", default="manual")
    p_feedback_ingest.add_argument("--decision", choices=["promote_for_work", "park", "reject", "needs_lyrics", "needs_chord", "needs_vocal_chop"], default="park")
    p_feedback_ingest.add_argument("--note", default="")
    p_feedback_ingest.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=True)
    ...
    p_paper.add_argument("--force-candidate", action="store_true", help="Write candidate immediately (equivalent to --no-dry-run)")
    ...
    p_creative_hb.add_argument("--explain", action="store_true")
    sub.add_parser("steering-vector", help="Create or show the AUTO2 steering vector")
    p_note_vector = sub.add_parser("note-vector-refresh", help="Refresh AUTO2 note meaning vectors from scoped reviewed/private sources")
    p_note_vector.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=False)
    p_seed_select = sub.add_parser("seed-select", help="Select an autonomous seed")
    p_seed_select.add_argument("--kind", choices=["song", "paper", "blocker", "memento"], required=True)
    p_auto_generate = sub.add_parser("autonomous-generate", help="Generate one autonomous review-gated candidate")
    p_auto_generate.add_argument("--kind", choices=["song", "paper"], required=True)
    p_auto_generate.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=True)
    p_auto_heartbeat = sub.add_parser("autonomous-heartbeat", help="Run AUTO2 vector-steered heartbeat")
    p_auto_heartbeat.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=True)
    ...
    p_ops_health = sub.add_parser("ops-health", help="Run end-to-end operational health checks")
    p_ops_health.add_argument("--strict", action="store_true")
    sub.add_parser("golden-path-smoke", help="Run Telegram-to-artifact golden path smoke tests")
    sub.add_parser("telegram-e2e-smoke", help="Alias of golden-path-smoke")
    sub.add_parser("qmd-vault-roundtrip", help="Validate reviewed/gold memory roundtrip into QMD retrieval")
    sub.add_parser("cron-health", help="Validate cron contract, outputs, and launcher stability")
    p_rollback_drill = sub.add_parser("rollback-drill", help="Render rollback drill plan")
    p_rollback_drill.add_argument("--dry-run", action="store_true")
    sub.add_parser("health-scorecard", help="Write state/ops/health_scorecard.json")
    sub.add_parser("daily-review", help="Write state/ops/daily_review.md ritual scaffold")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "normalize-event":
        from .normalize.source_normalizer import normalize_event, normalize_event_payload

        if args.text is not None:
            result = normalize_event_payload(source=args.source, text=args.text, kind=args.kind, dry_run=args.dry_run)
        else:
            result = normalize_event(args.source, dry_run=args.dry_run)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "feature-vectorize":
        from .features.feature_vector import create_feature_vector

        result = create_feature_vector(args.event_id, dry_run=args.dry_run)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "silver-to-candidate":
        from .enrichment.silver_to_candidate import create_candidate_from_event

        result = create_candidate_from_event(args.event_id, dry_run=args.dry_run)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "enrich-candidate":
        from .enrichment.candidate_enricher import enrich_candidate

        result = enrich_candidate(args.candidate_id, dry_run=args.dry_run)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "gold-readiness":
        from .enrichment.gold_readiness import evaluate_gold_readiness

        result = evaluate_gold_readiness(args.candidate_id, dry_run=args.dry_run)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "compile-gold":
        from .gold.gold_compiler import compile_gold_candidate

        result = compile_gold_candidate(args.candidate_id, dry_run=args.dry_run)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "gold-audit":
        from .gold.gold_audit import build_gold_audit

        result = build_gold_audit()
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "gold-rehab-audit":
        from .gold_rehab.corpus_audit import run_corpus_audit

        result = run_corpus_audit(dry_run=args.dry_run, limit=args.limit)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "gold-rehab-score":
        from .gold_rehab.corpus_audit import run_corpus_audit

        result = run_corpus_audit(dry_run=args.dry_run, limit=args.limit)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "gold-rehab-plan-frontmatter":
        from .gold_rehab.corpus_audit import run_corpus_audit
        from .governance_utils import read_jsonl
        from .gold_rehab.frontmatter_patch_plan import frontmatter_patch_candidates_path

        audit = run_corpus_audit(dry_run=args.dry_run, limit=args.limit)
        result = {
            "ok": audit.get("ok", False),
            "dry_run": args.dry_run,
            "frontmatter_patch_candidates": read_jsonl(frontmatter_patch_candidates_path()) if not args.dry_run else [],
            "corpus_audit": audit.get("corpus_audit"),
        }
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "gold-rehab-plan-append":
        from .gold_rehab.corpus_audit import run_corpus_audit
        from .governance_utils import read_jsonl
        from .gold_rehab.semantic_enrichment import semantic_enrichment_candidates_path

        audit = run_corpus_audit(dry_run=args.dry_run, limit=args.limit)
        result = {
            "ok": audit.get("ok", False),
            "dry_run": args.dry_run,
            "semantic_enrichment_candidates": read_jsonl(semantic_enrichment_candidates_path()) if not args.dry_run else [],
            "corpus_audit": audit.get("corpus_audit"),
        }
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "gold-rehab-apply-safe":
        from .gold_rehab.auto_append import apply_safe_enrichment

        result = apply_safe_enrichment(risk_max=args.risk_max, batch_size=args.batch_size)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "gold-rehab-review-needed":
        from .gold_rehab.review_needed import review_needed_summary

        result = review_needed_summary()
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "gold-rehab-discuss-later":
        from .gold_rehab.discuss_later import discuss_later_summary

        result = discuss_later_summary()
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "gold-rehab-rollback":
        from .gold_rehab.rollback_patch import rollback_patch_dry_run

        result = rollback_patch_dry_run(args.patch_id)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "gold-rehab-status":
        from .governance_utils import read_jsonl
        from .gold_rehab.patch_ledger import patch_ledger_path
        from .gold_rehab.review_needed import review_needed_path
        from .gold_rehab.discuss_later import discuss_later_path

        result = {
            "ok": True,
            "patch_count": len(read_jsonl(patch_ledger_path())),
            "review_needed_count": len(read_jsonl(review_needed_path())),
            "discuss_later_count": len(read_jsonl(discuss_later_path())),
        }
        _print(result)
        return 0
    if args.command == "training-candidates":
        from .training.training_manifest import build_training_candidates

        result = build_training_candidates()
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "training-export-manifest":
        from .training.training_manifest import build_training_manifest
    ...
    if args.command == "runtime-smoke":
        ...
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "soul-manifest":
        from .soul.manifest import write_soul_manifest

        result = write_soul_manifest()
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "soul-health":
        from .soul.health import write_soul_health

        result = write_soul_health()
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "soul-rehydrate":
        from .soul.rehydrate import run_soul_rehydrate

        if args.write == args.dry_run:
            _print({"ok": False, "reason": "choose-exactly-one-of---dry-run-or---write"})
            return 1
        result = run_soul_rehydrate(dry_run=args.dry_run, write=args.write)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "heartbeat-root-audit":
        from .soul.health import build_heartbeat_root_audit

        result = build_heartbeat_root_audit()
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "qmd-soul-audit":
        from .soul.health import build_qmd_soul_audit

        result = build_qmd_soul_audit()
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "heartbeat-readiness":
        from .orchestration.heartbeat_readiness import build_heartbeat_readiness

        result = build_heartbeat_readiness(strict=args.strict, write=True, run_dry_runs=True)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "heartbeat-dry-run-all":
        from .orchestration.heartbeat_dry_run import run_heartbeat_dry_run_all

        result = run_heartbeat_dry_run_all()
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "heartbeat-router-test":
        from .orchestration.telegram_router import heartbeat_router_test

        result = heartbeat_router_test(args.message)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "next-due-jobs":
        from .orchestration.telegram_router import next_due_jobs

        result = next_due_jobs()
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "cron-plan":
        from .orchestration.cron_plan import write_cron_plan

        result = write_cron_plan()
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "cron-render":
        from .orchestration.cron_render import render_cron_disabled

        if not args.disabled:
            _print({"ok": False, "reason": "cron-render-requires---disabled"})
            return 1
        result = render_cron_disabled()
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "cron-verify":
        from .orchestration.cron_render import verify_cron_plan
    ...
    if args.command == "song-feedback":
        ...
        return 0
    if args.command == "feedback-ingest":
        from .creative.songforge import record_song_feedback

        if args.dry_run:
            _print({"ok": True, "dry_run": True, "song_id": args.song_id, "decision": args.decision, "no_output_reason": "dry_run_preview"})
            return 0
        _print(record_song_feedback(args.song_id, args.decision, notes=args.note))
        return 0
    if args.command == "paper-onboarding":
        ...
        dry_run = False if args.force_candidate else bool(args.dry_run)
        _print(create_onboarding_pack(args.topic, dry_run=dry_run))
        return 0
    ...
    if args.command == "creative-heartbeat":
        ...
        _print(run_creative_heartbeat(dry_run=args.dry_run, explain=args.explain))
        return 0
    if args.command == "steering-vector":
        from .autonomy.steering_vector import steering_vector_health, write_steering_vector

        result = write_steering_vector()
        _print({**result, "health": steering_vector_health(result.get("steering_vector"))})
        return 0 if result.get("ok") else 1
    if args.command == "note-vector-refresh":
        from .autonomy.note_vector import build_note_vector

        result = build_note_vector(write=not args.dry_run)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "seed-select":
        from .autonomy.seed_selector import select_seed

        result = select_seed(args.kind, write=True)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "autonomous-generate":
        from .autonomy.autonomous_generation import generate_autonomous_candidate

        result = generate_autonomous_candidate(args.kind, dry_run=args.dry_run)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "autonomous-heartbeat":
        from .autonomy.autonomous_generation import run_autonomous_heartbeat

        result = run_autonomous_heartbeat(dry_run=args.dry_run)
        _print(result)
        return 0 if result.get("ok") else 1
    ...
    if args.command == "ops-health":
        from .orchestration.ops_health import run_ops_health

        result = run_ops_health(strict=args.strict, write=True)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "golden-path-smoke":
        from .orchestration.golden_path_smoke import run_golden_path_smoke

        result = run_golden_path_smoke(write=True)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "telegram-e2e-smoke":
        from .orchestration.golden_path_smoke import run_telegram_e2e_smoke

        result = run_telegram_e2e_smoke(write=True)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "qmd-vault-roundtrip":
        from .orchestration.ops_health import run_qmd_vault_roundtrip

        result = run_qmd_vault_roundtrip(strict=False, write=True)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "cron-health":
        from .orchestration.cron_health import build_cron_health

        result = build_cron_health(write=True)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "rollback-drill":
        from .orchestration.rollback_drill import run_rollback_drill

        result = run_rollback_drill(dry_run=True, write=True)
        _print(result)
        return 0 if result.get("ok") else 1
    if args.command == "health-scorecard":
        from .orchestration.health_scorecard import build_health_scorecard

        result = build_health_scorecard(write=True)
        _print({"ok": True, "scorecard": result})
        return 0
    if args.command == "daily-review":
        from .orchestration.health_scorecard import write_daily_review

        result = write_daily_review(write=True)
        _print(result)
        return 0 if result.get("ok") else 1
