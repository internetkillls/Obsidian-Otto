...
def run_health_repair(
    *,
    ...
    owner_decision = decide_gateway_owner()
    add_step("gateway-owner-decision", owner_decision)
    if (fresh or not gateway_probe.get("ok")) and owner_decision.get("active") in {"native", "unknown"}:
    ...
    elif not gateway_probe.get("ok"):
        add_step("openclaw-gateway-restart", {"ok": True, "skipped": True, "reason": "wsl-active-or-conflict"})
