...
The launcher is an operator entrypoint, not the A-B-C loop itself. It may trigger or observe A-B-C work, but its own state under `state/launcher/` remains separate from the Cowork-to-Otto bridge and from canonical Otto-Realm writeback.
