...
## Status
**MCP Fabric deployment in progress** — Obsidian read-only MCP is scaffolded, `config/docker.yaml` currently enables Docker/MCP, and Obsidian CLI MCP remains deferred until a real external CLI-capable backend is selected.
...
> This retain/temporary split follows the canonical control-plane architecture in `docs/architecture.md`.

## Phase 1: MCP Fabric Deployment (current)

### 1a. Docker Infrastructure
- [x] docker-compose.yml extended — obsidian-mcp service added
- [x] packages/obsidian-mcp/ built
- [x] config/docker.yaml updated — `enabled: true`
- [x] .env has MCP env vars (OBSIDIAN_VAULT_PATH already set)
...
- [x] Obsidian MCP container starts and responds to stdio ping (2026-04-17)
